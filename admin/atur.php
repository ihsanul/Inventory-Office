...
$path = "../foto/".$fotobaru;
 if(move_uploaded_file($tmp, $path)){
	...
	$query = mysqli_query($koneksi, "UPDATE tb_admin SET username='$username',password='$pass',nama_admin='$nama',jk_admin='$jk',foto='$fotobaru' WHERE id_admin='$id'");
	if (!$query){echo "Gagal edit data";
	}else {echo "Berhasil edit data";}
 }
}else{
	$query = mysqli_query($koneksi, "UPDATE tb_admin SET username='$username',password='$pass',nama_admin='$nama',jk_admin='$jk' WHERE id_admin='$id'");
	if (!$query){echo "Gagal edit data";
	}else {echo "Berhasil merubah data";}
}
